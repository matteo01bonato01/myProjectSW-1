...
public class GestioneUtente {
	...
	void prenota_cancella(final String user, final String password) {
		...
		try {
			...
			while(rs.next()) {
				jmi = new JMenuItem(rs.getString("ID").trim() + ". " + rs.getString("NOME_EVENTO").trim()
						+ " - " + rs.getString("NOME_LOCATION").trim());
		        menu.add(jmi);		
			}
			...
			public void actionPerformed(ActionEvent e){
				...
				try {
					...
					if(estrai != null){
						...
						int idInvio = Integer.parseInt(num[0]);
						...
						if(idInvio > 0) {
							...
							delEvento(idInvio, user, password);
						}
					} 
				} catch (Exception e1) {
					// TODO Auto-generated catch block
		...
		try {
        	...
        	/*
        	 * 2 ore per farlo funzionare: devo escludere le tuple in cui compare il login
        	...
        	stmt.execute("create or replace view VIEW" + user + "PRENOTABLE "
        			+ "AS select P.ID_EVENTO, E.NOME_EVENTO, L.NOME_LOCATION "
        			+ "from LOCATION as L, EVENTO as E, PRENOTAZIONE as P, UTENTEREGISTRATO as UR "
					+ "where L.NOME_LOCATION=E.NOME_LOCATION and E.ID = P.ID_EVENTO and P.LOGIN_UR=UR.LOG "
					+ "and P.LOGIN_UR != '" + user + "' "
					+ "and P.ID_EVENTO NOT IN (select ID_EVENTO from VIEW" + user + "SUBSCRIBED);");
			...
			while(rs2.next()) {
				jmi2 = new JMenuItem(rs2.getString("ID_EVENTO").trim() + ". " + rs2.getString("NOME_EVENTO").trim()
						+ " - " + rs2.getString("NOME_LOCATION").trim());
		        menu2.add(jmi2);
			}
		} catch(SQLException ex) {
			...
			public void actionPerformed(ActionEvent e){
				...
				try {
					...
					if(estrai != null) {
						...
						if(idInvio >0) {
							framePrenotami.setVisible(false);
							addEvento(idInvio, user, password);
						}
					}
				} catch (Exception e1) {
					// TODO Auto-generated catch block
